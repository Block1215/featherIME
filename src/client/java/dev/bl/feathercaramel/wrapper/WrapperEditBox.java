...
public final class WrapperEditBox extends AbstractIMEWrapper {
    ...
    @Override
    public Rect getRect() {
        // cursorPos は accesswidener で開放済み (getCursorPosition は 1.19.3 に存在)
        final int cursor = Math.min(wrapped.cursorPos, wrapped.value.length());
        final int   xWidth = wrapped.font.width(wrapped.value.substring(0, cursor));
        final float x      = xWidth + wrapped.x + (wrapped.bordered ? 4 : 0);
        final float y      = wrapped.font.lineHeight + wrapped.y
                             + (wrapped.bordered ? ((wrapped.height - 8) / 2.0f) : 0);
        return new Rect(x, y, wrapped.width, wrapped.height);
    }
